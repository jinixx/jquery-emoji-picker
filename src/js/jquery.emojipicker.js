  ...
  $.extend(EmojiPicker.prototype, {
    ...
    addPickerIcon: function() {
      ...
      if (this.settings.button) {
        ...
        this.$el.width(objectWidth).css("box-sizing", "content-box");

        this.$wrapper = this.$el
          .wrap("<div class='emojiPickerIconWrap'></div>")
          .parent();
        this.$icon = $('<div></div>') // class="emojiPickerIcon"
          .height(iconHeight)
          .width(iconHeight)
          .addClass(this.settings.iconColor)
          .css('backgroundColor', this.settings.iconBackgroundColor);
        var buttonPosition = ["left", "right"];
        if ($.inArray(this.settings.buttonPosition, buttonPosition) > -1) {
          if (this.settings.buttonPosition == "right") {
            this.$wrapper.append(this.$icon.addClass("emojiPickerIcon"));
            this.$el.css("paddingRight", this.$icon.outerWidth());
          } else {
            this.$wrapper.prepend(this.$icon.addClass("emojiPickerIcon-left"));
            this.$el.css("paddingLeft", this.$icon.outerWidth());
          }
        }
      }
    ...
    listen: function() {
      ...
      if (this.settings.button) {
        ...
        this.$wrapper.find('.emojiPickerIcon,.emojiPickerIcon-left')
